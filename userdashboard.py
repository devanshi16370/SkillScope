...
import chatbot
from notification import show_notifications, add_notification
...
def view_demand_skill():
    ...
    plt.pie(skill_count.values(), labels=skill_count.keys(), autopct='%0.2f%%', startangle=0, colors=plt.cm.Paired.colors)
...
def view_demand_jobs():
    ...
    plt.figure(figsize=(30, 30))
    plt.pie(job_count.values(), labels=job_count.keys(), autopct='%0.2f%%', startangle=0, colors=plt.cm.Paired.colors)
...
def user_dashboard(user_id):
    ...
    tk.Label(left_frame, text="User Dashboard", fg="white", bg="#2C3E50", font=("Arial", 14, "bold"), pady=15).pack()
    ...
    create_sidebar_button("📊 View Demanding Skills", view_demand_skill)
    create_sidebar_button("📊 View Demanding Jobs", view_demand_jobs)
    ...
    def apply_selected_job(event=None):
        ...
        job_title = job_details[2]

        conn_apply = sqlite3.connect('SkillScope.db')
        cursor_apply = conn_apply.cursor()

        cursor_apply.execute("SELECT recruiter_id FROM jobs WHERE job_id = ?", (job_id,))
        recruiter_row = cursor_apply.fetchone()
        ...
        cursor_apply.execute("SELECT username FROM users WHERE user_id = ?", (user_id,))
        user_row = cursor_apply.fetchone()
        username = user_row[0] if user_row else "A User"
        ...
        try:
            add_notification(user_id, "You applied for a job. Status: Pending.", role="user")
            add_notification(recruiter_id, f"{username} applied for the position '{job_title}'.", role="recruiter")
    ...
    tree.bind("<Double-1>", apply_selected_job)
